...
/**
 * This example illustrates how to generate ML-DSA key pairs The pair will then
 * be used to generate a
 * self-signed certificate and to sign/verify some data.
 */
public class SignatureMLDSADemo {
    ...
    public static void main(String[] args) {
        ...
        try {
             ...
             * no. to use.
             * Load that via "new FileInputStream(ksFileName)" instead of
             * ByteArrayInputStream. Save
        ...
        try {
            ...
            /*
             * The KeyPairGenerator class is used to determine the type of KeyPair being
             * generated. For
             * more information concerning the algorithms available in the Luna provider
             * please see the
             * Luna Development Guide. For more information concerning other providers,
             * please read the
             * documentation available for the provider in question.
             */
            System.out.println("Generating ML-DSA Keypair");
            /*
             * The KeyPairGenerator.getInstance method also supports specifying providers as
             * a parameter
             * to the method. Many other methods will allow you to specify the provider as a
             * parameter.
             */
            keyGen = KeyPairGenerator.getInstance("ML-DSA", provider);
            /*
             * ML-DSA keys need to know what curve to use. If you know the curve ID to use
             * you can specify
             * it directly. In the Luna Provider all supported curves are defined in
             * LunaECCurve
             */
            NamedParameterSpec spec = NamedParameterSpec.ML_DSA_65;
        ...
        try {
            ...
            /*
             * Sign/Verify operations like Encrypt/Decrypt operations can be performed in
             * either
             * singlepart or multipart steps. Single part Signing and Verify examples are
             * given in this
             * code. Multipart signatures use the Signature.update() method to load all the
             * bytes and then
             * invoke the Signature.sign() method to get the result. For more information
             * please see the
             * class documentation for the java.security.Signature class with respect to the
             * version of
             * the JDK you are using.
             */
            System.out.println("Signing encrypted text");
            // sig = Signature.getInstance("ML-DSA", provider);
            sig = Signature.getInstance("ML-DSAwithSHA256", provider);
            // sig = Signature.getInstance("ML-DSAwithSHA512", provider);
            // sig = Signature.getInstance("ML-DSAwith3_224", provider);
            // sig = Signature.getInstance("ML-DSAwithSHA3_384", provider);
            // sig = Signature.getInstance("ML-DSAwithSHAKE128", provider);
            // sig = Signature.getInstance("ML-DSAwithSHAKE256", provider);
            ...
            try {
                cert = LunaCertificateX509.SelfSign(keyPair, "CN=ML-DSA Sample Cert", serialNum, notBefore,
                        notAfter);
            } catch (InvalidKeyException | CertificateEncodingException ex) {
                ex.printStackTrace();
                System.exit(-1);
            }
